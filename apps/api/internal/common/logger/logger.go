...
func (logger *Logger) setupBaseLogger(ctx context.Context) *slog.Logger {
	...
	return logger.instance.With(
		slog.String("requestId", ctx.Value(middleware.RequestIDKey).(string)),
		slog.String(string(LoggerContextRemoteAddrKey), ctx.Value(LoggerContextRemoteAddrKey).(string)),
