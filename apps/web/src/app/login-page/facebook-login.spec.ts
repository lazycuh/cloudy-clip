...
describe('Facebook login', () => {
  ...
  it('Disable facebook login button after clicking it', async () => {
    apiRequestMockServer.resetHandlers(
    ...
    const loginButton = renderResult.container.querySelector('[aria-label="Log in with Facebook"]')!;
  ...
  it('Shows a notification with default message when opening consent page fails', async () => {
    apiRequestMockServer.resetHandlers(
      http.get(`${__ORCHESTRATOR_URL__}/v1/oauth2/facebook/url`, async () => {
        await delayBy(64);
    ...
    expect(
      screen.getByText('There was an unknown error trying to redirect to Facebook. Please try again later.')
    ).toBeInTheDocument();
    expect(ProgressService.prototype.close).toHaveBeenCalledOnce();

    assertCalledOnceWith(Logger.prototype.error, 'failed to open facebook consent page', {
      message: 'expected',
      payload: {
  ...
  it('Navigate to /my/account page after successful login and there is no intercepted page', async () => {
    ...
    await logInWithOauth2Provider('FACEBOOK', HttpResponse.json({ payload: TEST_USER_WITH_LITE_MONTHLY_SUBSCRIPTION }));
  ...
  it('Show error notification when login fails', async () => {
    ...
    expect(
      screen.getByText('Unable to log in using your selected Facebook account. Please try again later.')
    ).toBeInTheDocument();
  });
});
