...
if [ $# -eq 0 ]; then
    ...
    TRADE_TIMELINE_EXECUTION_PROFILE=ci go test -v -race -coverprofile=coverage.out -covermode=atomic -coverpkg=$COVER_PACKAGES $TEST_PACKAGES

    go tool cover -html=coverage.out -o coverage.html

elif [ "$1" == "-no-coverage" ]; then
    TRADE_TIMELINE_EXECUTION_PROFILE=ci go test -v $TEST_PACKAGES
fi
