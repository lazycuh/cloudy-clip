...
func TestDiscordLoginEndpoint(t1 *testing.T) {
	test.Integration(t1, func(testServer *httptest.Server) {
		t1.Run("1. returns correct authorization URL", func(t2 *testing.T) {
			...
			discordAuthUrl, err := url.Parse(responseBody["payload"].(string))
			if err != nil {
				panic(err)
			}
			...
			require.Equal(
				t2,
				environment.Config.Oauth2DiscordClientId,
				discordAuthUrl.Query().Get("client_id"),
			)

			redirectUri, err := url.PathUnescape(discordAuthUrl.Query().Get("redirect_uri"))
			if err != nil {
				panic(err)
			}
			require.Equal(
				t2,
				fmt.Sprintf("%s/login/oauth2/discord", environment.Config.AccessControlAllowOrigin),
				redirectUri,
			)

			scope, err := url.PathUnescape(discordAuthUrl.Query().Get("scope"))
			if err != nil {
				panic(err)
			}
			require.Equal(
				t2,
				"email identify openid",
				scope,
			)

			require.NotEmpty(
				t2,
				discordAuthUrl.Query().Get("state"),
			)
		...
		t1.Run("9. return 400 if user has already logged in with a different oauth2 provider before", func(t2 *testing.T) {
			...
			facebookLoginResponse, _ := test.SendPostRequest(
				t2,
				testServer,
			...
			require.Equal(t2, http.StatusOK, facebookLoginResponse.StatusCode)
			...
			discordLoginResponse, discordLoginResponseBody := test.SendPostRequest(
				t2,
				testServer,
				"/api/v1/oauth2/discord/me/sessions?code=code&state=state",
				nil,
			...
			require.Equal(t2, http.StatusBadRequest, discordLoginResponse.StatusCode)

			discordLoginResponseBodyPayload := discordLoginResponseBody["payload"].(map[string]any)

			require.Equal(
				t2,
				"oauth2 login is not allowed",
				discordLoginResponseBody["message"],
			)
			require.Equal(t2, "Oauth2LoginNotAllowedException", discordLoginResponseBodyPayload["code"])
			require.Equal(
				t2,
				map[string]any{
					"provider": "FACEBOOK",
				...
				discordLoginResponseBodyPayload["extra"],
			)
		})
